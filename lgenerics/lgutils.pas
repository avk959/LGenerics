...
type
  ...
  public
    ...
    function  Unwrap(out aValue: T): Boolean; inline;
...
var
  ...
  function PopCntSui(sui: SizeUInt): SizeInt; inline;
...
begin
  sui := sui - (sui shr 1) and c1;
  sui := sui and c2 + (sui shr 2) and c2;
  Result := SizeInt(((((sui + sui shr 4) and c3)*c4) shr 24) and $3f);
{$ENDIF CPUX86_HAS_POPCNT}
{$ELSE}
begin
  Result := SizeInt(PopCnt(sui));
...
function TGOptional<T>.Unwrap(out aValue: T): Boolean;
begin
  Result := Assigned;
  if Result then
    aValue := FValue;
...
begin
  ...
  case SIZE div 8 of
    0: Result := 0;
    1: Result := PopCnt(PQWord(p)^);
    2: Result := PopCnt(PQWord(p)^) + PopCnt(PQWord(p+8)^);
    3: Result := PopCnt(PQWord(p)^) + PopCnt(PQWord(p+8)^) + PopCnt(PQWord(p+16)^);
  else // 4
    Result := PopCnt(PQWord(p)^) + PopCnt(PQWord(p+8)^) + PopCnt(PQWord(p+16)^) + PopCnt(PQWord(p+24)^);
  end;
  case SIZE mod 8 of
    0: ;
    1: Inc(Result, PopCnt(DWord(p[OFFSET])));
    2: Inc(Result, PopCnt(DWord(PWord(p+OFFSET)^)));
...
begin
  ...
  case SIZE mod 8 of
    0: ;
    1: Inc(Result, PopCntD(p[OFFSET]));
    2: Inc(Result, PopCntD(PWord(p+OFFSET)^));
...
begin
  ...
  for I := 0 to Pred(SIZE div 4) do
    Inc(Result, PopCnt(PDWord(p+I*4)^));
  case SIZE mod 4 of
    0: ;
    1: Inc(Result, PopCnt(DWord(p[OFFSET])));
    2: Inc(Result, PopCnt(DWord(PWord(p+OFFSET)^)));
