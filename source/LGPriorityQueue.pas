...
{$ENDIF PAIRHEAP_ENABLE_PAGEDNODEMANAGER}
  ...
  { TGLiteCompareblePairHeapMax implements maximizing priority queue;
      it assumes that type T has defined comparision operators }
  generic TGLiteCompareblePairHeapMax<T> = record
  private
  type
    PGLiteCompareblePairHeapMax = ^TGLiteCompareblePairHeapMax;
...
{$ENDIF PAIRHEAP_ENABLE_PAGEDNODEMANAGER}
  ...
  type
    ...
    TReverse = record
    private
      FHeap: PGLiteCompareblePairHeapMax;
    public
      function GetEnumerator: TReverseEnumerator; inline;
    end;
  ...
  private
    ...
    procedure CheckEmpty; inline;
    procedure AccessEmptyError; inline;
    class operator  Initialize(var h: TGLiteCompareblePairHeapMax);
    class operator  Finalize(var h: TGLiteCompareblePairHeapMax);
    class operator  Copy(constref aSrc: TGLiteCompareblePairHeapMax; var aDst: TGLiteCompareblePairHeapMax);
    class function  NodeMerge(L, R: PNode): PNode; static; inline; //inline ???
    class function  TwoPassMerge(aNode: PNode): PNode; static;
    class procedure CutNode(aNode: PNode); static;
  ...
  type
    ...
    function  Remove(aHandle: THandle): T;                          // amortized O(logN)
  { note: after Merge all handles from aHeap will remain valid }
    function  Merge(aHeap: TGLiteCompareblePairHeapMax): SizeInt;   // O(1)
    property  Count: SizeInt read FCount;
    property  Capacity: SizeInt read GetCapacity;
  end;
...
{ TGLiteCompareblePairHeapMax.TNode }

procedure TGLiteCompareblePairHeapMax.TNode.AddChild(aNode: PNode);
begin
  if aNode <> nil then
    begin
      aNode^.Prev := @Self;
...
procedure TGLiteCompareblePairHeapMax.TNode.ClearLinks;
begin
  Sibling := nil;
  Child := nil;
  Prev := nil;
...
function TGLiteCompareblePairHeapMax.TNode.Successor: PNode;
begin
  Result := Sibling;
  if Result <> nil then
    while Result^.Child <> nil do
...
function TGLiteCompareblePairHeapMax.TNode.Predecessor: PNode;
begin
  Result := Child;
  if Result <> nil then
    while Result^.Sibling <> nil do
...
{ TGLiteCompareblePairHeapMax.TEnumerator }

function TGLiteCompareblePairHeapMax.TEnumerator.GetCurrent: T;
begin
  Result := FCurrNode^.Data;
end;

function TGLiteCompareblePairHeapMax.TEnumerator.MoveNext: Boolean;
var
  NextNode: PNode = nil;
begin
  if FCurrNode <> nil then
...
procedure TGLiteCompareblePairHeapMax.TEnumerator.Reset;
begin
  FCurrNode := nil;
  FInCycle := False;
end;

{ TGLiteCompareblePairHeapMax.TReverseEnumerator }

function TGLiteCompareblePairHeapMax.TReverseEnumerator.GetCurrent: T;
begin
  Result := FCurrNode^.Data;
end;

function TGLiteCompareblePairHeapMax.TReverseEnumerator.MoveNext: Boolean;
var
  NextNode: PNode = nil;
begin
  if FCurrNode <> nil then
...
procedure TGLiteCompareblePairHeapMax.TReverseEnumerator.Reset;
begin
  FCurrNode := nil;
  FInCycle := False;
end;

{ TGLiteCompareblePairHeapMax.TReverse }

function TGLiteCompareblePairHeapMax.TReverse.GetEnumerator: TReverseEnumerator;
begin
  Result := FHeap^.GetReverseEnumerator;
end;
...
function TGLiteCompareblePairHeapMax.GetCapacity: SizeInt;
begin
  Result := Count + FNodeManager.FreeCount;
end;

function TGLiteCompareblePairHeapMax.GetReverseEnumerator: TReverseEnumerator;
begin
  Result.FCurrNode := nil;
  Result.FRightmost := FindRightmost;
  Result.FInCycle := False;
...
function TGLiteCompareblePairHeapMax.FindLeftmost: PNode;
begin
  Result := FRoot;
  if Result <> nil then
    while Result^.Child <> nil do
...
function TGLiteCompareblePairHeapMax.FindRightmost: PNode;
begin
  Result := FRoot;
  if Result <> nil then
    while Result^.Sibling <> nil do
...
procedure TGLiteCompareblePairHeapMax.RemoveNodeWithChilds(aNode: PNode);
var
  OldPrev, OldNode, CurrNode: PNode;
  FromSibling: Boolean = False;
begin
...
procedure TGLiteCompareblePairHeapMax.ClearTree;
begin
  RemoveNodeWithChilds(FRoot);
  FRoot := nil;
end;

function TGLiteCompareblePairHeapMax.NewNode(constref aValue: T): PNode;
begin
  Result := FNodeManager.NewNode;
  Result^.Data := aValue;
  Inc(FCount);
...
procedure TGLiteCompareblePairHeapMax.DisposeNode(aNode: PNode);
begin
  if aNode <> nil then
    begin
      aNode^ := Default(TNode);
...
procedure TGLiteCompareblePairHeapMax.RootMerge(aNode: PNode);
begin
  FRoot := NodeMerge(FRoot, aNode);
  if FRoot <> nil then
    FRoot^.Prev := nil;
...
function TGLiteCompareblePairHeapMax.DequeueItem: T;
var
  OldRoot: PNode;
begin
  OldRoot := FRoot;
...
procedure TGLiteCompareblePairHeapMax.UpdateNode(aNode: PNode; constref aValue: T);
var
  c: SizeInt;
begin
  c := Compare(aValue, aNode^.Data);
  if c <> 0 then
    begin
      aNode^.Data := aValue;
      if c > 0 then
...
procedure TGLiteCompareblePairHeapMax.ExtractNode(aNode: PNode);
begin
  if aNode <> FRoot then
    begin
      CutNode(aNode);
...
function TGLiteCompareblePairHeapMax.RemoveNode(aNode: PNode): T;
begin
  ExtractNode(aNode);
  Result := aNode^.Data;
  DisposeNode(aNode);
...
procedure TGLiteCompareblePairHeapMax.CheckEmpty;
begin
  if Count = 0 then
    AccessEmptyError;
end;

procedure TGLiteCompareblePairHeapMax.AccessEmptyError;
begin
  raise ELGAccessEmpty.Create(SECantAccessEmpty);
end;

class operator TGLiteCompareblePairHeapMax.Initialize(var h: TGLiteCompareblePairHeapMax);
begin
  h := Default(TGLiteCompareblePairHeapMax);
end;

class operator TGLiteCompareblePairHeapMax.Finalize(var h: TGLiteCompareblePairHeapMax);
begin
  h.Clear;
end;

class operator TGLiteCompareblePairHeapMax.Copy(constref aSrc: TGLiteCompareblePairHeapMax;
  var aDst: TGLiteCompareblePairHeapMax);
var
  v: T;
begin
  System.FillChar(aDst, SizeOf(aDst), 0);
...
class function TGLiteCompareblePairHeapMax.NodeMerge(L, R: PNode): PNode;
begin
  if L <> nil then
    if R <> nil then
      if L^.Data >= R^.Data then
...
{$IFDEF ENABLE_TWOPASSMERGE_RECURSION}
class function TGLiteCompareblePairHeapMax.TwoPassMerge(aNode: PNode): PNode; // recursive
var
  Sibling, NextSibling: PNode;
begin
  if (aNode <> nil) and (aNode^.Sibling <> nil) then
...
{$ELSE ENABLE_TWOPASSMERGE_RECURSION}
class function TGLiteCompareblePairHeapMax.TwoPassMerge(aNode: PNode): PNode;
var
  CurrNode, NextNode: PNode;
begin
  Result := nil;
...
class procedure TGLiteCompareblePairHeapMax.CutNode(aNode: PNode);
begin
  if aNode^.Sibling <> nil then
    aNode^.Sibling^.Prev := aNode^.Prev;
  if aNode^.Prev^.Child = aNode then
...
class function TGLiteCompareblePairHeapMax.Compare(constref L, R: T): SizeInt;
begin
  if L > R then
    Result := 1
  else
...
class function TGLiteCompareblePairHeapMax.DoCompare(constref L, R: T): SizeInt;
begin
  Result := Compare(L, R);
end;

function TGLiteCompareblePairHeapMax.Comparator: TComparator;
begin
  Result := @DoCompare;
end;

function TGLiteCompareblePairHeapMax.GetEnumerator: TEnumerator;
begin
  Result.FCurrNode := nil;
  Result.FLeftmost := FindLeftmost;
  Result.FInCycle := False;
...
function TGLiteCompareblePairHeapMax.Reverse: TReverse;
begin
  Result.FHeap := @Self;
end;

function TGLiteCompareblePairHeapMax.ToArray: TArray;
var
  I: SizeInt = 0;
  v: T;
begin
...
procedure TGLiteCompareblePairHeapMax.Clear;
begin
  ClearTree;
  FNodeManager.Clear;
end;

function TGLiteCompareblePairHeapMax.IsEmpty: Boolean;
begin
  Result := Count = 0;
end;

function TGLiteCompareblePairHeapMax.NonEmpty: Boolean;
begin
  Result := Count <> 0;
end;

procedure TGLiteCompareblePairHeapMax.EnsureCapacity(aValue: SizeInt);
begin
  FNodeManager.EnsureFreeCount(aValue - Capacity);
end;

procedure TGLiteCompareblePairHeapMax.TrimToFit;
begin
  if Count > 0 then
    FNodeManager.ClearFreeList
  else
...
procedure TGLiteCompareblePairHeapMax.Enqueue(constref aValue: T);
begin
  RootMerge(NewNode(aValue));
end;

function TGLiteCompareblePairHeapMax.Dequeue: T;
begin
  CheckEmpty;
  Result := DequeueItem;
end;

function TGLiteCompareblePairHeapMax.TryDequeue(out aValue: T): Boolean;
begin
  Result := NonEmpty;
  if Result then
    aValue := DequeueItem;
...
function TGLiteCompareblePairHeapMax.Peek: T;
begin
  CheckEmpty;
  Result := FRoot^.Data;
end;

function TGLiteCompareblePairHeapMax.TryPeek(out aValue: T): Boolean;
begin
  Result := NonEmpty;
  if Result then
    aValue := FRoot^.Data;
...
function TGLiteCompareblePairHeapMax.Insert(constref aValue: T): THandle;
var
  p: PNode;
begin
  p := NewNode(aValue);
...
function TGLiteCompareblePairHeapMax.PeekHead: THandle;
begin
  CheckEmpty;
  Result := {%H-}THandle(FRoot);
end;

function TGLiteCompareblePairHeapMax.TryPeekHead(out aValue: THandle): Boolean;
begin
  Result := NonEmpty;
  if Result then
    aValue := {%H-}THandle(FRoot);
...
function TGLiteCompareblePairHeapMax.Value(aHandle: THandle): T;
begin
  Result := {%H-}PNode(aHandle)^.Data;
end;

procedure TGLiteCompareblePairHeapMax.Update(aHandle: THandle; constref aValue: T);
begin
  CheckEmpty;
  UpdateNode({%H-}PNode(aHandle), aValue);
end;

function TGLiteCompareblePairHeapMax.Remove(aHandle: THandle): T;
begin
  CheckEmpty;
  Result := RemoveNode({%H-}PNode(aHandle));
end;

function TGLiteCompareblePairHeapMax.Merge(aHeap: TGLiteCompareblePairHeapMax): SizeInt;
begin
  Result := aHeap.Count;
  if Result > 0 then
    begin
